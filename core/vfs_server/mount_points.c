...
static size_t iterate_over_mount_point(size_t mount_point_address, void *data, bool (*fn)(void *, size_t, bool, size_t *)) {
    ...
    struct mount_point *mount_point = (struct mount_point *) syscall_invoke(mount_point_address, SIZE_MAX, UNTYPED_LOCK, 0);
...
static void mount_point_link(size_t thread_id, struct directory_info *info, struct ipc_message *message) {
    struct link_args args = {
        .to_send = {
            .buffer = {FD_LINK},
            .capabilities = {{THREAD_STORAGE_SLOT(thread_id, 7), SIZE_MAX}, FD_LINK_FD(*message), FD_LINK_NAME_ADDRESS(*message)},
            .to_copy = 7
        },
...
static void mount_point_unlink(size_t thread_id, struct directory_info *info, struct ipc_message *message) {
    struct link_args args = {
        .to_send = {
            .buffer = {FD_UNLINK},
            .capabilities = {{THREAD_STORAGE_SLOT(thread_id, 7), SIZE_MAX}, FD_UNLINK_NAME_ADDRESS(*message)},
            .to_copy = 3
        },
...
static void mount_point_open(const struct state *state, struct directory_info *info, struct ipc_message *message) {
    ...
    struct open_args args = {
        ...
        .info = {
            ...
            .can_modify_namespace = info->can_modify_namespace,
            .inode = info->inode,
    ...
    if ((FD_OPEN_FLAGS(*message) & (OPEN_CREATE | OPEN_EXCLUSIVE)) == (OPEN_CREATE | OPEN_EXCLUSIVE)) {
        ...
        FD_OPEN_FLAGS(*message) &= (uint8_t) ~OPEN_CREATE;

        syscall_invoke(info->mount_point_address, SIZE_MAX, UNTYPED_UNLOCK, 0);
...
size_t mount(struct directory_info *info, size_t directory_fd, uint8_t flags) {
    ...
    if (mounted_list_index == SIZE_MAX) {
        syscall_invoke(namespace_address, SIZE_MAX, UNTYPED_UNLOCK, 0);
        return ENOMEM;
    }
